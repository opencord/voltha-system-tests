...
# Duration of collecting pm data (metrics)
# example: -v CollectionsDuration:500s
${CollectionsDuration}    120s
...
Check Default Metrics All ONUs
    ...
    ${onu_interval_list}    ${longest_interval}=    Get Onu Metrics Interval
    log    get metrics ${longest_interval}s    console=yes
    ${longest_interval}=    evaluate    ((${longest_interval}*2)+(${longest_interval}*0.2))
    ${Kafka_Records}=    Get Metrics    ${longest_interval}s
    ...
    FOR    ${Index}    IN RANGE    0    ${RecordsLength}
        ...
        ${result}=    Validate Slice Data   ${slice}    ${Kafka_Records}
        Set Previous Record    ${Index}    ${slice}
    END
    ${result}=    Validate Number of Checks
...
Check User Onu Metrics
    ...
    # prepare user pm data list
    ${METRIC_LIST}=     Create User Metric List
    Set Suite Variable    ${METRIC_LIST}
    ${onu_interval_list}    ${longest_interval}=    Get Onu Metrics Interval    ${METRIC_LIST}
    Set And Validate Metrics Interval    ${METRIC_LIST}
    # second call of keyword to get the current longest interval
    ${dummy_list}    ${longest_interval}=    Get Onu Metrics Interval    ${METRIC_LIST}
    ${longest_interval}=    evaluate    ((${longest_interval}*2)+(${longest_interval}*0.2))
    ${Kafka_Records}=    Get Metrics    ${longest_interval}s
    ${RecordsLength}=    Get Length    ${Kafka_Records}
    FOR    ${Index}    IN RANGE    0    ${RecordsLength}
        ...
        Set Previous Record    ${Index}    ${slice}
    END
    ${result}=    Validate Number of Checks    ${METRIC_LIST}
    Set And Validate Previous Onu Metrics Interval    ${onu_interval_list}
    [Teardown]    Run Keywords    Clean Metric Dictionary
    ...    AND    Run Keyword If    ${logging}    Collect Logs
...
Create User Metric List
    [Documentation]    create user metric list of metrics to test
    ...                currently it is 'hard coded', will be filled by yaml file in future
    ${UNI_Status_dict}=     Create Dictionary    group=UNI_Status    interval=${ONU_UNI_STATUS_INTERVAL}
    ${PON_Optical_dict}=    Create Dictionary    group=PON_Optical   interval=${ONU_PON_OPTICAL_INTERVAL}
    ${metric_list}   Create List    ${UNI_Status_dict}
    ...                             ${PON_Optical_dict}
    [return]    ${metric_list}
