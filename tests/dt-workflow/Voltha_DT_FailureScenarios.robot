...
Verify restart openolt-adapter container after subscriber provisioning for DT
    ...
    ${podName}    Set Variable     ${adapter-pod-name}
...
Verify openolt adapter restart before subscriber provisioning for DT
    ...
    Scale K8s Deployment by Pod Label    ${NAMESPACE}    app    ${adapter-pod-name}    0
    Wait Until Keyword Succeeds    ${timeout}    2s    Pods Do Not Exist By Label    app    ${adapter-pod-name}
    # Scale up the open OLT adapter deployment and make sure both it and the ofagent deployment are back
    Scale K8s Deployment by Pod Label    ${NAMESPACE}    app    ${adapter-pod-name}    1
    Wait Until Keyword Succeeds    ${timeout}    2s
    ...    Check Expected Available Deployment Replicas By Pod Label     ${NAMESPACE}    app    ${adapter-pod-name}    1
