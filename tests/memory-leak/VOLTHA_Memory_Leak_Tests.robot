...
Memory Leak Test Openonu Go Adapter
    ...
    #Sleep 60 seconds to give prometheus a chance to update the memory consumptions - it is a workaround
    Sleep   60s
    ${start_mem_consumption}=    utility.get_memory_consumptions  ${prometheusaddr}:${prometheusport}   adapter-open-onu  voltha
    Run Keyword If    ${print2console}    Log    \r\nMemory consumptions Start ${start_mem_consumption}    console=yes
    ${time}=    Get Time
    ...
    FOR    ${I}    IN RANGE    1    ${iterations} + 1
        ...
        Run Keyword And Ignore Error    Remove Flows all ONUs
    ...
    ...    AND    Teardown Test
    ...    AND    Wait Until Keyword Succeeds    600    10s
...
Memory Leak Test Openolt Adapter
    ...
    ...    Attention: Due VOL-4703 is not corrected errors in Sanity Test as well as Validate ONU Devices will be ignored!
    ...               This is a temporaly workaround only! Has to be checked after introduction of voltha-go-controller.
    ...               For same reason errors in Delete Devices In Voltha and Test Empty Device List will be ignored.
    ...
    FOR    ${I}    IN RANGE    1    ${iterations} + 1
        ...
        Run Keyword And Ignore Error    Wait Until Keyword Succeeds    ${timeout}    1s    Test Empty Device List
        Sleep    20s
