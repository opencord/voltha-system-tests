...
Test that assured BW is allocated as needed on the PON
    ...
    # Case 2: Verify for VOD and HSIA service combined
    ${out_file_1_2}=    Set Variable    ${CURDIR}/../../tests/data/out_1_2
    Run Iperf3 Test Client in Background    ${src_1_2}    server=${dst_1_2['dp_iface_ip_qinq']}
    ...    args=-t 30 -p 5201    out_file=${out_file_1_2}
    ${out_file_1_4}=    Set Variable    ${CURDIR}/../../tests/data/out_1_4
    Run Iperf3 Test Client in Background    ${src_1_4}    server=${dst_1_4['dp_iface_ip_qinq']}
    ...    args=-t 30 -p 5202    out_file=${out_file_1_4}
    # Wait for the above iperf commands to finish
    Sleep    35s
    ${out_1_2}=    Read Output File on Remote System    ${src_1_2}    ${out_file_1_2}
    ${out_1_4}=    Read Output File on Remote System    ${src_1_4}    ${out_file_1_4}
    ${actual_upstream_bw_used_1_2}=    Evaluate    ${out_1_2['end']['sum_received']['bits_per_second']}/1000
    ${pct_limit_up_1_2}=    Evaluate    100*${actual_upstream_bw_used_1_2}/${limiting_bw_us_1_2}
    Should Be True    ${pct_limit_up_1_2} >= ${lower_margin_pct}
    ...    The upstream bandwidth guarantee was not met (${pct_limit_up_1_2}% of resv)
    ${actual_upstream_bw_used_1_4}=    Evaluate    ${out_1_4['end']['sum_received']['bits_per_second']}/1000
    ${pct_limit_up_1_4}=    Evaluate    100*${actual_upstream_bw_used_1_4}/(${limiting_bw_us_1_4}-${limiting_bw_us_1_2})
    Should Be True    ${pct_limit_up_1_4} >= ${lower_margin_pct}
    ...    The upstream bandwidth guarantee was not met (${pct_limit_up_1_4}% of resv)
