...
Test that the BW is limited to GIR
    ...
    # Find the ONU with service type and bandwidth profile as required for this test
    ${req_service_type}=    Set Variable    'voip'
    ${req_bw_profile}=    Set Variable    'TCONT_TYPE1_200Mbps_Fixed_ForVOIP'
    ${matched}=    Set Variable    False
    FOR    ${I}    IN RANGE    0    ${num_all_onus}
        ${src}=    Set Variable    ${hosts.src[I]}
        ${dst}=    Set Variable    ${hosts.dst[I]}
        ${service_type}=    Get Variable Value    ${src['service_type']}    "null"
        Continue For Loop If    '${service_type}' != ${req_service_type}
        ...
        ...    '${service_type}' == ${req_service_type} and ${bandwidth_profile_name} == ${req_bw_profile}    True    False
        Exit For Loop If    ${matched}
    END
    Pass Execution If    '${matched}' != 'True'
    ...    Skipping test: No ONU found with service ${req_service_type} and bw profile ${req_bw_profile}
