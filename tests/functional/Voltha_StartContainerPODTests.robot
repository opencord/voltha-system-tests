...
Remove Helm Charts
    ...
    FOR    ${helm_chart}    IN    @{List_Helm_Charts}
        ...
        Run Keyword If    ${with_onos} or ('${helmchartname}'!='onos')    Remove VOLTHA Helm Charts    ${helmchartname}    ${namespace}
...
Restart Voltha Adapters Simulated
    ...
    ${cmd}	Catenate
    ...    helm install -f ${kind_voltha_dir}/minimal-values.yaml    --set services.etcd.service=etcd.default.svc
    ...    --set services.etcd.port=2379 --set kafka_broker=kafka.default.svc:9092
    ...
    ...    --set defaults.log_level=WARN --namespace voltha --name sim    onf/voltha-adapter-simulated
    ${rc}    Run And Return Rc    ${cmd}
    Should Be Equal as Integers    ${rc}    0
...
Restart Voltha Adapters Open OLT
    ...
    ${cmd}	Catenate
    ...    helm install -f ${kind_voltha_dir}/minimal-values.yaml    --create-namespace
    ...
    #...    --set services.kafka.adapter.service=kafka.default.svc --set services.kafka.adapter.port=9092
    #...    --set services.kafka.cluster.service=kafka.default.svc --set services.kafka.cluster.port=9092
    #...    --set defaults.log_level=WARN --namespace voltha --name open-olt    onf/voltha-adapter-openolt
    ${rc}    Run And Return Rc    ${cmd}
    Should Be Equal as Integers    ${rc}    0
...
Restart Voltha Adapters Open ONU
    ...
    #...    --set services.kafka.adapter.service=kafka.default.svc --set services.kafka.adapter.port=9092
    #...    --set services.kafka.cluster.service=kafka.default.svc --set services.kafka.cluster.port=9092
    #...    --set replicas.adapter_open_onu=1  --set defaults.log_level=WARN --namespace voltha --name open-onu
    #...       onf/voltha-adapter-openonu
    ${rc}    Run And Return Rc    ${cmd}
    Should Be Equal as Integers    ${rc}    0
...
Restart Voltha
    ...
    ${cmd}	Catenate
    ...    helm install -f ${kind_voltha_dir}/minimal-values.yaml   --create-namespace --set therecanbeonlyone=true
    ...
    #...    helm install -f ${kind_voltha_dir}/minimal-values.yaml   --set therecanbeonlyone=true
    #...    --set services.etcd.service=etcd.default.svc
    #...    --set services.etcd.port=2379 --set kafka_broker=kafka.default.svc:9092
    #...    --set services.kafka.adapter.service=kafka.default.svc --set services.kafka.adapter.port=9092
    #...    --set services.kafka.cluster.service=kafka.default.svc --set services.kafka.cluster.port=9092
    #...    --set services.controller[0].service=onos-onos-classic-0.onos-onos-classic-hs.default.svc
    #...    --set services.controller[0].port=6653  --set defaults.log_level=WARN --namespace voltha --name voltha
    #...       onf/voltha
    ${rc}    Run And Return Rc    ${cmd}
    Should Be Equal as Integers    ${rc}    0
...
Restart ONOS
    ...
    ...    helm install -f ${kind_voltha_dir}/minimal-values.yaml
    ...
    #...    helm install -f ${kind_voltha_dir}/minimal-values.yaml
    #...    --set image.repository=voltha/voltha-onos,image.tag=master,replicas=1,atomix.replicas=0
    #...    --set defaults.log_level=WARN --namespace default --name onos  onos/onos-classic
    #...    ELSE    Catenate
    #...    helm install -f ${kind_voltha_dir}/minimal-values.yaml
    #...    --set image.repository=voltha/voltha-onos,image.tag=master,replicas=1,atomix.replicas=0
    #...    --set defaults.log_level=WARN --namespace default --name onos --version ${onos_version}  onos/onos-classic
    ${rc}    Run And Return Rc    ${cmd}
    Should Be Equal as Integers    ${rc}    0
...
Configure ONOS RADIUS Connection
    ...
    #...    sed -e s/:RADIUS_SVC:/radius.default.svc/g -e s/:RADIUS_PORT:/1812/
    #...    ${kind_voltha_dir}/onos-files/onos-aaa.json | curl --fail -sSL --user karaf:karaf -X POST
    #...    http://127.0.0.1:8181/onos/v1/network/configuration/apps/org.opencord.aaa
    #...    -H Content-type:application/json -d@-
    ${rc}    Run And Return Rc    ${cmd}
