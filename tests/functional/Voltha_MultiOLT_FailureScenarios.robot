...
Verify OLT after rebooting physically - MultipleOLT
    ...
    Pass Execution If    ${olt_count} > 1    Skipping test: just one instance of ONOS
    ...
    # validate that the other OLTs
    FOR   ${I}    IN RANGE    1    ${olt_count}
        ${olt_serial_number}=    Get From Dictionary    ${list_olts}[${I}]    sn
        ${olt_device_id}=    Get OLTDeviceID From OLT List    ${olt_serial_number}
        Verify ping is succesful for ONUs not on this OLT     ${num_all_onus}    ${olt_device_id}
    END
