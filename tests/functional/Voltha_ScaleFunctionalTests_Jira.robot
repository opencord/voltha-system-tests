...
Allocate DHCP To All ONU Devices 
	...
	${onu_device_id}=    Get Device ID From SN    ${src['onu']}
        ${onu_port}=    Run Keyword And Continue On Failure    Wait Until Keyword Succeeds	${timeout}    2s	Get ONU Port in ONOS    ${src['onu']}    ${of_id}
        Run Keyword And Continue On Failure    Wait Until Keyword Succeeds    ${timeout}    2	Execute ONOS CLI Command    ${k8s_node_ip}    ${ONOS_SSH_PORT}	volt-add-subscriber-access ${of_id} ${onu_port}
    END
...
Validate Total Number Of DHCP Allocations
    ...
    [Tags]  VOL-1824
    #validate total number of DHCP allocations  
    Wait Until Keyword Succeeds  ${long_timeout}  20s  Validate DHCP Allocations  ${k8s_node_ip}  ${ONOS_SSH_PORT}        16
    ...
    FOR    ${I}    IN RANGE    0    ${num_onus}
        ...
        ${onu_device_id}=    Get Device ID From SN    ${src['onu']}
        ${onu_port}=    Run Keyword And Continue On Failure    Wait Until Keyword Succeeds	${timeout}    2s	Get ONU Port in ONOS    ${src['onu']}    ${of_id}
	Wait Until Keyword Succeeds  ${long_timeout}  20s  Validate Subscriber DHCP Allocation	${k8s_node_ip}	${ONOS_SSH_PORT}	${onu_port}	
    END
