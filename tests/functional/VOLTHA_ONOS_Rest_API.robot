...
*** Test Cases ***
Test ONOS Rest API
    [Documentation]   Test of ONOS Rest API instead of ONOS SSH
    [Tags]    OnosRestApi
    [Setup]    Run Keywords    Start Logging    OnosRestApi
    ...        AND             Setup
    Run Keyword If    ${has_dataplane}    Clean Up Linux
    Run Keyword If    ${print2console}    Log    \r\nStart ONOS Rest API.    console=yes
    Wait for Ports in ONOS for all OLTs    ${ONOS_REST_IP}    ${ONOS_REST_PORT}    ${num_all_onus}    BBSM    ${timeout}
...
Teardown Suite
    ...
    Run Keyword If    ${pausebeforecleanup}    Log    Suite Teardown will be continued...    console=yes
    ...
    Wait for Ports in ONOS for all OLTs    ${ONOS_REST_IP}    ${ONOS_REST_PORT}    0    BBSM    ${timeout}
    #Run Keyword If    ${logging}    Collect Logs
...
Teardown Test
    ...
    Run Keyword If    ${pausebeforecleanup}    Import Library    Dialogs
    Run Keyword If    ${pausebeforecleanup}    Pause Execution    Press OK to continue with clean up!
    Run Keyword If    ${pausebeforecleanup}    Log    Test Teardown will be continued...    console=yes
    Run Keyword If    ${teardown_device}       Delete All Devices and Verify
    # delete etcd MIB Template Data
    Delete MIB Template Data    ${INFRA_NAMESPACE}
