...
Data plane verification with user defined bandwidth profiles
    ...
    ...    physical pod.Hence skipping this test in BBSIM
    ...
    FOR    ${I}    IN RANGE    0    ${num_onus}
        ...
        ...    iperf3 -c ${dst['dp_iface_ip_qinq']} -J --logfile auto_out_rg1 & sleep 10
        ...    ${src['ip']}    ${src['user']}    ${src['pass']}    ${src['container_type']}    ${src['container_name']}
        Sleep    10s
        ${actual_bw_used_in_bits_up}=    Login And Run Command On Remote System
        ...    cat auto_out_rg1 | grep "sum_sent" -A 7 | grep "bits_per_second" | grep -o '[0-9.]*'
        ...    ${src['ip']}    ${src['user']}    ${src['pass']}    ${src['container_type']}
        ...    ${src['container_name']}
        ${actual_bw_used_in_bits_up}=    convert to integer    ${actual_bw_used_in_bits_up}
        ${actual_upstream_bw_used}=    Evaluate   ${actual_bw_used_in_bits_up}*0,001
        ${actual_bw_used_in_bits_down}=    Login And Run Command On Remote System
        ...    cat auto_out_rg1 | grep "sum_received" -A 7 | grep "bits_per_second" | grep -o '[0-9.]*'
        ...    ${src['ip']}    ${src['user']}    ${src['pass']}    ${src['container_type']}
        ...    ${src['container_name']}
        ${actual_dnstream_bw_used}=   Evaluate     ${actual_bw_used_in_bits_down}*0.001
        Login And Run Command On Remote System    pkill iperf; rm -f auto_out_rg1
        ...    ${src['ip']}    ${src['user']}    ${src['pass']}    ${src['container_type']}    ${src['container_name']}
        Run Keyword if    ${limiting_bw_value_upstream} > ${actual_upstream_bw_used}
        ...    The upstream traffic exceed the limit
        Run Keyword if    ${limiting_bw_value_dnstream} > ${actual_dnstream_bw_used}
