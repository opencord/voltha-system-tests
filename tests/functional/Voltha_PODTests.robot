...
${allow_lower_margin}    0.93    # Allow 7% under the limit
...
Data plane verification with user defined bandwidth profiles
    ...
    FOR    ${I}    IN RANGE    0    ${num_onus}
        ...
        # Stream from RG to server: TCP test, 30 seconds
        ${result}=    Run Iperf3 Test Client    ${src}    server=${dst['dp_iface_ip_qinq']}    args=-t 30
        ${actual_upstream_bw_used}=    Evaluate    ${result['end']['sum_received']['bits_per_second']}/1000
        Log    Upstream: profile ${limiting_bw_value_upstream} Kbps, measured ${actual_upstream_bw_used} Kbps

        # Stream from server to RG: TCP test, 30 seconds
        ${result}=    Run Iperf3 Test Client    ${src}    server=${dst['dp_iface_ip_qinq']}    args=-t 30 -R
        ${actual_dnstream_bw_used}=    Evaluate    ${result['end']['sum_received']['bits_per_second']}/1000
