...
Test ONU Delete and Auto-Discovery
    ...
    FOR    ${I}    IN RANGE    0    ${num_all_onus}
        ...
        ...    ${src['dp_iface_name']}    wpa_supplicant.conf    ${src['ip']}    ${src['user']}    ${src['pass']}
        ...    ${src['container_type']}    ${src['container_name']}    ${wpa_log}
        # LIMITATION: Skipping `aaa-users` verification as BBSim currently doesn't initiate re-authentication by itself
        Run Keyword If    ${has_dataplane}    Wait Until Keyword Succeeds    ${timeout}    2
