...
Validate parsing of data traffic through voltha using tech profile
    ...
    [Teardown]    None
    ${rc}    ${output}=    Run and Return Rc and Output
    ...    kubectl get pods -n voltha | grep etcd-cluster | awk 'NR==1{print $1}'
    Should Be Equal As Integers    ${rc}    0
    ${etcd_cluster_pod}=    Set Variable    ${output}
    ${rc}    ${output}=    Run and Return Rc and Output
    ...    kubectl exec -i -n voltha '${etcd_cluster_pod}' -- etcdctl del --prefix service/voltha/technology_profiles
    Should Be Equal As Integers    ${rc}    0
    Log    ${output}
    Delete Device and Verify
    ...
    Should Be Equal As Integers    ${rc}    0
    ${cmd_put_techprofile}=    Set Variable
    ...    jq -c . ${tech_profile_name}.json | kubectl exec -i -n voltha ${etcd_cluster_pod} -- etcdctl put service/voltha/technology_profiles/XGS-PON/64
