...
Data plane Bandwidth profile update verification
    ...
    [Tags]    non-critical    dataplane    BandwidthProfileUpdateTCP    VOL-2053
    [Setup]    Start Logging    BandwidthProfileUpdateTCP
    [Teardown]    Run Keywords    Collect Logs
    ...           AND    Stop Logging    BandwidthProfileUpdateTCP
...
Test ONU Delete and Auto-Discovery
    ...
    FOR    ${I}    IN RANGE    0    ${num_all_onus}
        ...
        Delete Device    ${onu_device_id}
        Run Keyword If    ${has_dataplane}    Verify ping is succesful except for given device
        ...    ${num_all_onus}    ${onu_device_id}
        # Remove Subscriber Access (To replicate ATT workflow)
        Wait Until Keyword Succeeds    ${timeout}    2s    Execute ONOS CLI Command    ${ONOS_SSH_IP}
        ...    ${ONOS_SSH_PORT}    volt-remove-subscriber-access ${of_id} ${onu_port}
        # Additional sleep to let subscriber delete process and onu re-discovery
        Sleep    15s
        # ONU Auto-Discovery
