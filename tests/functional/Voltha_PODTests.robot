...
Sanity E2E Test for OLT/ONU on POD
    ...
    Validate Authentication    True    ${src0['dp_iface_name']}    wpa_supplicant.conf    ${src0['ip']}
    ...     ${src0['user']}    ${src0['pass']}      ${src0['container_type']}    ${src0['container_name']}
    #Validate ONU authenticated in ONOS
    #Wait Until Keyword Succeeds    90s    2s    Verify Number of AAA-Users    ${k8s_node_ip}    ${ONOS_SSH_PORT}
    ...     ${num_onus}
    ${onu_port}=    Wait Until Keyword Succeeds    90s    2s    Get ONU Port in ONOS    ${onu_serial_number}   ${of_id}
    Wait Until Keyword Succeeds    60s    2s    Execute ONOS Command    ${k8s_node_ip}    ${ONOS_SSH_PORT}
    ...     volt-add-subscriber-access ${of_id} ${onu_port}
    # Perform dhclient and ping operations
    Validate DHCP and Ping    True    True    ${src0['dp_iface_name']}    ${src0['s_tag']}    ${src0['c_tag']}
    ...     ${dst0['dp_iface_ip_qinq']}     ${src0['ip']}    ${src0['user']}    ${src0['pass']}
    ...     ${src0['container_type']}    ${src0['container_name']}    ${dst0['dp_iface_name']}      ${dst0['ip']}
    ...     ${dst0['user']}    ${dst0['pass']}    ${dst0['container_type']}    ${dst0['container_name']}
    #Validate DHCP allocation in ONOS
    Wait Until Keyword Succeeds    60s    2s    Validate DHCP Allocations    ${k8s_node_ip}    ${ONOS_SSH_PORT}
    ...     ${num_onus}
...
Setup
    ...
    #validate olt states
    Wait Until Keyword Succeeds    60s    5s    Validate Device    ENABLED    ACTIVE   REACHABLE    ${olt_serial_number}
    #validate onu states
    Wait Until Keyword Succeeds    60s    5s    Validate Device    ENABLED    ACTIVE   REACHABLE    ${onu_serial_number}
...
Clean Up Linux
    ...
    Run Keyword And Ignore Error    Kill Linux Process    [w]pa_supplicant    ${src0['ip']}    ${src0['user']}
    ...     ${src0['pass']}    ${src0['container_type']}    ${src0['container_name']}
    Run Keyword And Ignore Error    Kill Linux Process    [d]hclient    ${src0['ip']}    ${src0['user']}
    ...     ${src0['pass']}    ${src0['container_type']}    ${src0['container_name']}
    Run Keyword If    '${dst0['ip']}' != '${None}'    Run Keyword And Ignore Error    Kill Linux Process    [d]hcpd
    ...     ${dst0['ip']}    ${dst0['user']}    ${dst0['pass']}    ${dst0['container_type']}   ${dst0['container_name']}
    Delete IP Addresses from Interface on Remote Host    ${src0['dp_iface_name']}    ${src0['ip']}    ${src0['user']}
    ...     ${src0['pass']}    ${src0['container_type']}    ${src0['container_name']}
