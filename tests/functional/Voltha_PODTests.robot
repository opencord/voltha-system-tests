...
Data plane verification with user defined bandwidth profiles
    ...
    FOR    ${I}    IN RANGE    0    ${num_onus}
        ...
        ...    jq -M '.end.sum_sent.bits_per_second' auto_out_rg1
        ...    ${src['ip']}    ${src['user']}    ${src['pass']}    ${src['container_type']}
        ...    ${src['container_name']}
        ${actual_bw_used_in_bits_up}=    Fetch From Left    ${output}    ${SPACE}
        ...
        ${output}=    Login And Run Command On Remote System
        ...    jq -M '.end.sum_received.bits_per_second' auto_out_rg1
        ...    ${src['ip']}    ${src['user']}    ${src['pass']}    ${src['container_type']}
        ...    ${src['container_name']}
        ${actual_bw_used_in_bits_down}=    Fetch From Left    ${output}    ${SPACE}
        ${actual_bw_used_in_bits_down}=    Convert To Integer    ${actual_bw_used_in_bits_down}
