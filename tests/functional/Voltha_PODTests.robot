...
Data plane verification with user defined bandwidth profiles
    ...
    FOR    ${I}    IN RANGE    0    ${num_onus}
        ...
        ...    cd iperf-3.7;./src/iperf3 -c ${dst['dp_iface_ip_qinq']} -J --logfile auto_out_rg1 & sleep 10
        ...    ${src['ip']}    ${src['user']}    ${src['pass']}    ${src['container_type']}    ${src['container_name']}
        Sleep    10s
        ${actual_bw_used_in_bits_up}=    Login And Run Command On Remote System
        ...    cat iperf-3.7/auto_out_rg1 | grep "sum_sent" -A 7 | grep "bits_per_second" | grep -o '[0-9.]*'
        ...
        ...    cat iperf-3.7/auto_out_rg1 | grep "sum_received" -A 7 | grep "bits_per_second" | grep -o '[0-9.]*'
        ...    ${src['ip']}    ${src['user']}    ${src['pass']}    ${src['container_type']}
        ...    ${src['container_name']}
