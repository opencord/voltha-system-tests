...
Data plane verification using TCP
    ...
    [Tags]    sanity    BW-profile-TCP    VOL-2052
    [Setup]    None
    [Teardown]    None
    ...
    FOR    ${I}    IN RANGE    0    ${num_onus}
        ...
        ${pct_of_limit_up}=    Evaluate    100*${actual_upstream_bw_used}/${limiting_bw_value_upstream}
        ${pct_of_limit_dn}=    Evaluate    100*${actual_dnstream_bw_used}/${limiting_bw_value_dnstream}
        Log    Upstream: profile ${limiting_bw_value_upstream} Kbps, measured ${actual_upstream_bw_used} Kbps (${pct_of_limit_up}%)
        Log    Downstream: profile ${limiting_bw_value_dnstream} Kbps, measured ${actual_dnstream_bw_used} Kbps (${pct_of_limit_dn}%)

        Should Be True    ${pct_of_limit_up} <= ${upper_margin_pct}
        ...    The upstream bandwidth exceeded the limit (${pct_of_limit_up}% of limit)
        Should Be True    ${pct_of_limit_dn} <= ${upper_margin_pct}
        ...    The downstream bandwidth exceeded the limit (${pct_of_limit_dn}% of limit)
        Should Be True    ${pct_of_limit_up} >= ${lower_margin_pct}
        ...    The upstream bandwidth guarantee was not met (${pct_of_limit_up}% of resv)
        Should Be True    ${pct_of_limit_dn} >= ${lower_margin_pct}
        ...    The downstream bandwidth guarantee was not met (${pct_of_limit_dn}% of resv)
    END

Data plane verification using UDP
    [Documentation]    Test bandwidth profile is met and not exceeded for each subscriber.
    ...    Assumes iperf3 and jq installed on client and iperf -s running on DHCP server
    [Tags]    sanity    BW-profile-UDP    VOL-2052
    [Setup]    None
    [Teardown]    None
    Pass Execution If   '${has_dataplane}'=='False'    Bandwidth profile validation can be done only in
    ...    physical pod.  Skipping this test in BBSIM.
