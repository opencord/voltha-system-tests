...
Delete All Devices and Verify
    ...
    # Fetch device list count
    ${rc}    ${count}=    Run and Return Rc and Output    ${VOLTCTL_CONFIG}; voltctl device list -q | wc -l
    Should Be Equal As Integers    ${rc}    0
    Log    ${count}

    # Clear devices from VOLTHA
    Run Keyword If    int(${count}) != 0    Disable All Devices In Voltha
    Run Keyword If    int(${count}) != 0    Wait Until Keyword Succeeds    ${timeout}    2s    Test All Devices Disabled In Voltha
    Run Keyword If    int(${count}) != 0    Delete All Devices In Voltha
    Run Keyword If    int(${count}) != 0    Wait Until Keyword Succeeds    ${timeout}    2s    Test Empty Device List

    # Clear devices from ONOS
    Remove All Devices From ONOS
    ...    http://karaf:karaf@${k8s_node_ip}:${ONOS_REST_PORT}

    # If we have to clear devices that means the OLT had to reboot, so we will
    # wait for it to rebot
    Run Keyword If    int(${count}) != 0 and ${has_dataplane}    Wait Until Keyword Succeeds    600s    5s
    ...    Openolt is Up    ${olt_ip}    ${olt_user}    ${olt_pass}
