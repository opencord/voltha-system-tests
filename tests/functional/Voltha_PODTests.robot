...
${dhcp_ip}    192.168.10.24
${dhcp_user}    volthadhcp
${dhcp_password}    volthadhcp
${dhcp_server_iface}    ens10
...
Validate parsing of data traffic through voltha using tech profile
    ...
    [Tags]    functional    TechProfile    notready    VOL-2054
    [Setup]    None
    [Teardown]    None
    ${rc}    ${output}=    Run and Return Rc and Output    kubectl get pods -n voltha | grep etcd-cluster | awk 'NR==1{print $1}'
    Should Be Equal As Integers    ${rc}    0
    ${etcd_cluster_pod}=    Set Variable    ${output}
    ${rc}    ${output}=    Run and Return Rc and Output    kubectl exec -i -n voltha '${etcd_cluster_pod}' -- etcdctl del --prefix service/voltha/technology_profiles
    Should Be Equal As Integers    ${rc}    0
    Log    ${output}
    Delete Device and Verify
    ...
    Should Be Equal As Integers    ${rc}    0
    ${cmd_put_techprofile}=    Set Variable    jq -c . ${tech_profile_name}.json | kubectl exec -i -n voltha ${etcd_cluster_pod} -- etcdctl put service/voltha/technology_profiles/XGS-PON/64
    ${rc}    ${output}=    Run and Return Rc and Output    ${cmd_put_techprofile}
    Should Be Equal As Integers    ${rc}    0
    Log    ${output}
    ...
    FOR    ${I}    IN RANGE    0    ${num_onus}
        ...
        Run Keyword If    ${has_dataplane}    Wait Until Keyword Succeeds    ${timeout}    2s
        ...    Create upstream traffic with each pbit and capture at other end    ${dst['dp_iface_ip_qinq']}    ${src['dp_iface_name']}
        ...    ${rg_mac}    ${dhcp_mac}    0    udp    dp=9999    0    ${src['ip']}    ${src['user']}    ${src['pass']}
        ...    ${src['container_type']}    ${src['container_name']}
        Run Keyword If    ${has_dataplane}    Wait Until Keyword Succeeds    ${timeout}    2s
        ...    Create downstream traffic with each pbit and capture at other end    ${rg_ip}    ${src['dp_iface_name']}
        ...    ${rg_mac}    ${dhcp_mac}    0    udp    dp=9999    0    ${src['ip']}    ${src['user']}    ${src['pass']}
        ...    ${src['container_type']}    ${src['container_name']}
    END
    ${rc}    ${output}=    Run and Return Rc and Output    kubectl exec -i -n voltha '${etcd_cluster_pod}' -- etcdctl del --prefix service/voltha/technology_profiles
