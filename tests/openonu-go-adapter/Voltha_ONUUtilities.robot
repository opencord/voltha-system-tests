...
Validate Vlan Rules In Etcd
    ...
    ...                In case of a passed dictionary containing set_vids these will be checked for not equal to
    ...                current set-vid.
    [Arguments]    ${nbofcookieslice}=1    ${reqmatchvid}=4096    ${prevvlanrules}=${NONE}
    ...
    FOR    ${INDEX}    IN RANGE    0    ${length}
        ...
        ${prevsetvid}=    Set Variable If    ${prevvlanrules} is not ${NONE}    ${prevvlanrules['${pononuuniid}']}
        Run Keyword If    ${prevvlanrules} is not ${NONE}
