...
Perform ONU MIB Template Compare OMCI Baseline and Extended Message
    ...
    ${BaseTxArFrames}=    Run Keyword If    ${rc} == 0   Get From Dictionary   ${OMCI_counter_dict}   BaseTxArFrames
    ...    ELSE    Set Variable    -1
    ${BaseRxAkFrames}=    Run Keyword If    ${rc} == 0   Get From Dictionary   ${OMCI_counter_dict}   BaseRxAkFrames
    ...    ELSE    Set Variable    -1
    Run Keyword If    ${rc} == 0    Should Be Equal As Integers   ${BaseTxArFrames}   ${BaseRxAkFrames}
    ...                             Number of baseline Rx and Tx frames do not match!
    ...
    ${rc}=    Set Variable If    ${BaseTxArFrames} == -1    -1    ${rc}
    ${ExtTxArFrames}=    Run Keyword If    ${rc} == 0    Get From Dictionary    ${OMCI_counter_dict}    ExtTxArFrames
    ...    ELSE    Set Variable    -1
    ${ExtRxAkFrames}=    Run Keyword If    ${rc} == 0    Get From Dictionary    ${OMCI_counter_dict}    ExtRxAkFrames
    ...    ELSE    Set Variable    -1
    Run Keyword If    ${rc} == 0    Should Be Equal As Integers   ${ExtTxArFrames}   ${ExtRxAkFrames}
    ...                             Number of extended Rx and Tx frames do not match!
    # check baseline and extended OMCI frames counter
    ${TxArFrames_compare}=    Run Keyword If    ${rc} == 0    Evaluate    ${BaseTxArFrames}*0.05 > ${ExtTxArFrames}
    ...    ELSE    Set Variable    True
    Should Be True    ${TxArFrames_compare}   Comparison of TxArFrames failed (${BaseTxArFrames}:${ExtTxArFrames})!
    ${RxAkFrames_compare}=    Run Keyword If    ${rc} == 0    Evaluate    ${BaseRxAkFrames}*0.05 > ${ExtRxAkFrames}
    ...    ELSE    Set Variable    True
