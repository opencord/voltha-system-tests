...
Perform ONU MIB Template Data Test
    ...
    ${status}    Evaluate    ${firstonustartup}*0.1>=${secondonustartup}
...
Kill BBSIM
    [Documentation]    This keyword kills the BBSIM and wait for BBSIM is up again.
    ${list_openonu_apps}   Create List    bbsim
    ${namespace}=    Set Variable    voltha
    ${adaptorname}=    Set Variable    bbsim
    # Restart Pod    ${namespace}    ${adaptorname}
    # workaround for repeating test -> kill bbsim
    Kill Adaptor    ${namespace}    ${adaptorname}
    # wait for BBSIM is up again
