...
Flows Test
    ...
    Run Keyword And Continue On Failure    Validate Etcd Vlan Rules Added Subscriber    defaultkvstoreprefix=${kvstoreprefix}
    ...
    ${flowsresult}=    Execute ONOS CLI Command use single connection    ${ONOS_SSH_IP}    ${ONOS_SSH_PORT}    flows -s
    log     ${flowsresult}
    Run Keyword And Continue On Failure    Validate Etcd Vlan Rules Removed Subscriber   defaultkvstoreprefix=${kvstoreprefix}
    FOR    ${J}    IN RANGE    0    ${num_olts}
        ${olt_serial_number}=    Set Variable    ${list_olts}[${J}][sn]
        ${of_id}=    Wait Until Keyword Succeeds    ${timeout}    15s    Validate OLT Device in ONOS
        ...    ${olt_serial_number}
...
Validate Etcd Vlan Rules Added Subscriber
    ...
    FOR    ${INDEX}    IN RANGE    0    ${length}
        ...
        ...    set_vid
        ${evalresult}=    Run Keyword If    "${workflow}"=="ATT"    Evaluate    2 <= ${setvid} <= 4095
        ...               ELSE IF           "${workflow}"=="DT"     Evaluate    ${setvid} == 4096
        ...               ELSE              Evaluate    2 <= ${setvid} <= 4095
...
Validate Etcd Vlan Rules Removed Subscriber
    ...
    FOR    ${INDEX}    IN RANGE    0    ${length}
        ...
        ...    ELSE IF    "${workflow}"=="DT"     Should Be Equal As Strings    ${result}    FAIL
        ...
        ...               ELSE IF           "${workflow}"=="DT"     Evaluate    "${result}" == "FAIL"
