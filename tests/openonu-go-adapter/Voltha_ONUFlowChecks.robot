...
Flows Test
    ...
    ...    Validate Etcd Vlan Rules Added Subscriber    defaultkvstoreprefix=${kvstoreprefix}
...
Validate Etcd Vlan Rules Added Subscriber
    ...
    FOR    ${INDEX}    IN RANGE    0    ${length}
        ...
        Validate Flow Params Vlan Rules    ${flowparams}    ${reqmatchvid}
    END
...
Validate Flow Params Vlan Rules
    ...
    ...                It checks the match_vid (=4096) and set_vid when subscriber are added.
    [Arguments]    ${flowparams}    ${reqmatchvid}=4096
    ${length}=    Get Length    ${flowparams}
    ${evalres}=    Run Keyword If    "${workflow}"=="TT"    Evaluate    ${length} >= 3
        ...                  ELSE    Evaluate    ${length} == 1
    Should Be True    ${evalres}    msg=wrong number of vlan rules (${length})!
    FOR    ${INDEX}    IN RANGE    0    ${length}
        ...
        ...    set_vid
        ${evalresult}=    Run Keyword If    "${workflow}"=="ATT"    Evaluate    900 <= ${setvid} <= 4095
        ...               ELSE IF           "${workflow}"=="DT"     Evaluate    ${setvid} == 4096
        ...               ELSE              Evaluate    (900 <= ${setvid} <= 4095) or (${setvid} == 55) or (${setvid} == 444)
