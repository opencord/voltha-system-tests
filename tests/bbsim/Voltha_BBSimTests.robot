...
Perform BBSim Sanity Test Per OLT
    ...
    FOR    ${I}    IN RANGE    0    ${num_all_onus}
        ...
        ...    Validate Subscriber DHCP Allocation    ${ONOS_SSH_IP}    ${ONOS_SSH_PORT}    ${onu_port}
        ...    AND    List ONUs    ${NAMESPACE}    ${bbsim_pod}
        # Perform Igmp Join (valid only for TT)
        Run Keyword If    "${workflow}"=="TT"
        ...    Run Keywords
        ...    List Service    ${NAMESPACE}    ${bbsim_pod}
        ...    AND    Sleep    5s
        ...    AND    JoinOrLeave Igmp    ${NAMESPACE}    ${bbsim_pod}    ${src['onu']}    join
        ...    AND    List Service    ${NAMESPACE}    ${bbsim_pod}
        ...    AND    List ONUs    ${NAMESPACE}    ${bbsim_pod}
        ...    AND    Sleep    5s
        ...    AND    Verify ONU in Groups    ${ONOS_SSH_IP}    ${ONOS_SSH_PORT}    ${of_id}    ${onu_port}
        # Perform Igmp Leave (valid only for TT)
        ...    AND    JoinOrLeave Igmp    ${NAMESPACE}    ${bbsim_pod}    ${src['onu']}    leave
        ...    AND    List Service    ${NAMESPACE}    ${bbsim_pod}
        ...    AND    List ONUs    ${NAMESPACE}    ${bbsim_pod}
    END
