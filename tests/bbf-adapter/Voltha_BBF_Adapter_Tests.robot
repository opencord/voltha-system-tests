...
Test Disable and Enable ONU for BBF
    [Documentation]    Validates the possibility to disable ONUs from the BBF Adapter (PlaceHolder)
    ...     Verify the disable state in voltha and in the BBF Adapter
    ...     Enable the ONUs from the BBF adapter, verify the enable stati in voltha 
    ...     and in BBF Adapter.
    ...
    FOR    ${I}    IN RANGE    0    ${num_all_onus}
        ...
        ${onu_reasons}=  Create List     tech-profile-config-delete-success
...
Test Disable and Enable OLT and validate ONUs for BBF
    [Documentation]    Validates the possibility to disable OLTs from the BBF Adapter (Placeholder)
    ...     Verify the disable state in voltha and in the BBF Adapter
    ...     Enable the OLTs from the BBF adapter, verify the enable state in voltha 
    ...     and in BBF Adapter. Also will be validate all the ONUS states.
    ...     It is also possible to verify a connectivity, with a no block test.
    ...
    END
    # Validate all ONUs after OLT Disable
    # Define states that the ONU reason need to have after disable of the OLT
    ...
    FOR    ${I}    IN RANGE    0    ${num_all_onus}
        ...
        ...    UNREACHABLE    ${src['onu']}    onu=True    onu_reason=${onu_reasons}
        # Verify in the BBF Adapter the State of the ONU
        Run Keyword And Continue On Failure    Wait Until Keyword Succeeds    ${timeout}    5s
    ...
    FOR   ${I}    IN RANGE    0    ${olt_count}
        ...
        ...    ${olt_serial_number}  ${olt_device_id}
    END
    # Validate ONUs after reenable the OLT
    # Define some states that the ONU reason need to have after OLT enable
    ${onu_reasons}=  Create List     omci-flows-pushed
...
Test Disable and Delete OLT for BBF
    ...
    [Teardown]    Run Keywords    Run Keyword If    ${logging}    Collect Logs
    ...           AND             Stop Logging    disableEnableOLTBBF
    Set Global Variable     ${supress_add_subscriber}
...
Test Delete and ReAdd OLT for BBF
    [Documentation]    Validates the possibility to Delete OLTs from the BBF Adapter (PlaceHolder)
...
Test Disable ONUs and OLT Then Delete ONUs and OLT for BBF
    [Documentation]   Validates states of OLT and ONUS adter Dsiable and Delete from the BBF Adapter (PlaceHolder)
    ...     Disable ONUs and OLTs, verify if the OLT and ONUs connected was really disable in voltha and in the BBF Adapter.
    ...     Delete ONUs and OLTs, verify if the ONUs and OLT was really deleted in voltha and BBF Adapter.
    [Tags]    bbfAdapterFunctionality    DisableDeleteONUOLTBBF
    [Setup]    Run Keywords    Start Logging    DisableDeleteONUOLTBBF
    [Teardown]    Run Keywords    Run Keyword If    ${logging}    Collect Logs
    ...           AND             Stop Logging    DisableDeleteONUOLTBBF
    Run Keyword     Setup    ${SOAK_TEST}
    Run Keyword If    ${has_dataplane}    Clean Up Linux
    # Define some states that the ONU reason can be
    FOR    ${I}    IN RANGE    0    ${num_all_onus}
...
Test ONU Delete and Auto-Discovery for BBF
    ...
    [Tags]    bbfAdapterFunctionalitySingleTest    ONUAutoDiscoveryBBF
