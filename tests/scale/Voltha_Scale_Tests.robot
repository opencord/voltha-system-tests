...
Flows validation in ONOS before subscriber provisioning
    ...
    # NOTE fail the test immediately if we're trying to check flows without provisioning them
    Should Be Equal   ${enableFlowProvisioning}     true
    Wait for all flows to in ADDED state    ${ONOS_SSH_IP}    ${ONOS_SSH_PORT}  ${workflow}    ${total_onus}    ${olt}    false
...
Flows validation in ONOS after subscriber provisioning
    ...
    # NOTE fail the test immediately if we're trying to check flows without provisioning them
    Should Be Equal   ${enableFlowProvisioning}     true
    Wait for all flows to in ADDED state    ${ONOS_SSH_IP}    ${ONOS_SSH_PORT}  ${workflow}    ${total_onus}    ${olt}    true
...
*** Keywords ***
Setup Suite
    [Documentation]    Create and enable OLT Devices
...
Teardown Suite
    [Documentation]    Disable and delete OLT Devices
