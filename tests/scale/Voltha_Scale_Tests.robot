...
Image Activation and Validation during ONUs Software Upgrade
    ...
    Pass Execution If    '${activate_on_success}'=='true'    Skipping test: Image already activated in download stage
    ${onu_device_ids}=    Get ONUs Device IDs from Voltha
    FOR    ${onu_device_id}    IN    @{onu_device_ids}
...
Image Commit and Validation during ONUs Software Upgrade
    ...
    Pass Execution If    '${commit_on_success}'=='true'    Skipping test: Image already committed in download or activate stage
    ${onu_device_ids}=    Get ONUs Device IDs from Voltha
    FOR    ${onu_device_id}    IN    @{onu_device_ids}
