...
Library           robot_grpc.Dmi    WITH NAME    dmi2
...
*** Test Cases ***
add and remove device in device manager
...
get set component inventory info
...
Suite Setup 
    ...
    ${suite_device_uuid}=    add device  dmi1    ${OLT_IP}    BBSim-BBSIM_OLT_0
...
Suite Teardown
    remove device   dmi1    BBSim-BBSIM_OLT_0
    dmi1.Connection Close
    search for active devices and remove it from device manager     dmi1
    search for active devices and remove it from device manager     dmi2
