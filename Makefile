...
bbsim-kind: ROBOT_PORT_ARGS ?= -v ONOS_REST_PORT:8181 -v ONOS_SSH_PORT:8101 -v OLT_PORT:50060
bbsim-kind: ROBOT_MISC_ARGS ?= -v num_onus:1 -v setup:bbsim -V $(ROOT_DIR)/tests/data/bbsim-kind.yaml -v ports_per_onu:4 -X
...
voltha-podtest: vst_venv
	...
	set -u ;\
	cd tests/functional ;\
	robot $(ROBOT_PORT_ARGS) $(ROBOT_TEST_ARGS) $(ROBOT_MISC_ARGS) $(ROBOT_OLT_SN) $(ROBOT_ONU_SN) Voltha_PODTests.robot
