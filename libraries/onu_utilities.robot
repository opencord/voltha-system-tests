...
Wait for all ONU Adapter Reconcile
    ...
    # - we check that all ONU state are set to reconciling. All of the ONUs will be set to reconciling
    # - we need the Ignore Error to continue, because it is very hard to catch reconciling oper-state for all ONUs
    # due a very small durtion of this oper-state (8-10ms)
    ${list_onus}    Create List
    Build ONU SN List    ${list_onus}
    ${length}=    Get Length    ${list_onus}
    Run Keyword And Ignore Error    Wait Until Keyword Succeeds    ${timeout}
    ...     0ms  Check all ONU OperStatus     ${list_onus}  RECONCILING
    # - then we wait that all ONU move to the next state
    ${afterReconcileLength}=    Get Length    ${list_onus}
    ${catchedReconcile}=    Evaluate    ${length} - ${afterReconcileLength}
    Log    For ${catchedReconcile} ONU(s) of ${length} ONU(s) Oper-State RECONCILING was catched.
    Build ONU SN List    ${list_onus}
    Run Keyword And Ignore Error    Wait Until Keyword Succeeds    ${timeout}
    ...     100ms  Check all ONU OperStatus     ${list_onus}  ${oper_status}
