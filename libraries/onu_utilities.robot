...
Wait for ONU Adapter Reconcile
    ...
    # - we check that the first ONU state is set to reconciling. All of the ONUs will be set to reconciling very quickly at that point
    Wait Until Keyword Succeeds    ${timeout}    100ms  Check ONU OperStatus     ${hosts.src[0]['onu']}  RECONCILING
...
Check ONU OperStatus
    ...
    ...    --format "{{.SerialNumber}}\t{{.AdminState}}\t{{.OperStatus}}\t{{.ConnectStatus}}\t{{.Reason}}" | grep -v SERIALNUMBER | grep ${sn}
