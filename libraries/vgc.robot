...
Create VGC Session
    Create Session    VGC    http://${ONOS_REST_IP}:${ONOS_REST_PORT}
...
Verify Subscriber Access Flows Added For ONU DT in VGC
    ...
    FOR    ${INDEX}    IN RANGE    0    ${length}
        ...
        ${matched}=     Run Keyword If  '${tableid}' == '0' and '${inport}' == '${onu_port}' and '${vlanvid}' == '4096' and '${outport}' == '1'   Set Variable    ${TRUE}   
        Exit For Loop If    ${matched}
    END
    Should Be Equal    ${matched}    ${TRUE}    No match for upstream table 0 flow  found
    ...
    FOR    ${INDEX}    IN RANGE    0    ${length}
        ...
        ${matched1}=     Run Keyword If   '${inport}' == '${onu_port}' and '${vlanvid}' == '4096' and '${outport}' == '${nni_port}' and '${subtype}' == 'VLAN_PUSH' and '${vlanid}' == '${s_tag}'   Set Variable    ${TRUE}
        Exit For Loop If    ${matched1}
    END
    Should Be Equal   ${matched1}     ${TRUE}    No match for upstream table 1 flow found
    ...
    FOR    ${INDEX}    IN RANGE    0    ${length}
        ...
        ${outport}=    Get Out Port From Flow    ${flow}
        ${subtype}=    Get Subtype From Flow   ${flow}
        ${matched2}=     Run Keyword If   '${inport}' == '${nni_port}' and '${vlanvid}' == '${s_tag}' and '${outport}' == '1' and '${subtype}' == 'VLAN_POP'   Set Variable    ${TRUE}
        Exit For Loop If    ${matched2}
    END
    Should Be Equal   ${matched2}     ${TRUE}    No match for downstream table 0  flow found
    ...
    FOR    ${INDEX}    IN RANGE    0    ${length}
        ...
        ${vlanvid}=    Get Vlan VId From Flow    ${flow}
        ${outport}=    Get Out Port From Flow    ${flow}
        ${matched3}=     Run Keyword If   '${inport}' == '${nni_port}' and '${vlanvid}' == '4096' and '${outport}' == '${onu_port}'   Set Variable    ${TRUE}
        Exit For Loop If    ${matched3}
    END
    Should Be Equal   ${matched3}     ${TRUE}    No match for  downstream table 1 flow  found
...
Get Vlan Id From Flow
    ...
    FOR    ${INDEX}    IN RANGE    0    ${len}
           ...
           ${type}=       Get From Dictionary    ${instructions}    type
           ${subtype}=    Run Keyword If    '${type}' == 'L2MODIFICATION'      Get From Dictionary    ${instructions}    subtype
           ${vlanId}=    Run Keyword If    '${type}' == 'L2MODIFICATION' and '${subtype}' == 'VLAN_ID'    Get From Dictionary    ${instructions}    vlanId
...
Verify Meters in VGC Ietf
    ...
    ${rate}    ${burst_size}   Get Meter Param In Details
    ...    ${meter_json_resp}  3
    ${matched}=    Set Variable If    '${rate}' == '${us_gir}' and '${burst_size}' == '0'  True    False
    Should Be True    ${matched}
    # Verify meter for upstream bandwidth profile
...
Get Upstream and Downstream Bandwidth Profile Name
    ...
    ${subscribername}=    Get From Dictionary    ${programmed_sub}    subscribers
    Log    ${subscribername}
    ${length}=    Get Length    ${subscribername}
    ${matched}=    Set Variable    False
    FOR    ${INDEX}    IN RANGE    0    ${length}
        ${value}=    Get From List    ${subscribername}    ${INDEX}
...
#*** Test Cases ***
#Test 
#   ${data}=     Validate OLT Device in VGC      BBSIM_OLT_1
#    Log To Console     ${data}  
