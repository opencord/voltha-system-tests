...
Wait for OpenOLT Devices flows
    ...
    # In the TT workflow we have multiple service, so we need to remove 7 flows per each UNI that are only on the ONU device
    ${ttFlows}=     Evaluate    ${beforeFlows} - (${uni_count} * 7)
