...
Global Teardown
    ...
    ${active_devices}=     Find Devices   dmi1
    ${size}=    Get Length  ${active_devices}
    Run Keyword If   ${size} != ${0}   Disable Devices      dmi1    ${active_devices}
    ${active_devices}=     Find Devices   dmi1
...
search for active devices and remove it from device manager
    [Documentation]     search for known/active devices and remove it from device manager
    [Arguments]    ${lib_instance}
    Run Keyword     ${lib_instance}.Connection Open    ${DEVICEMANAGER_IP}    ${DEVICEMANAGER_PORT}
    ${active_devices}=     Find Devices   ${lib_instance}
    ${size}=    Get Length  ${active_devices}
    Run Keyword If   ${size} != ${0}   Disable Devices      ${lib_instance}    ${active_devices}
