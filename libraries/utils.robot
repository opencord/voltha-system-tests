...
Create upstream traffic with each pbit and capture at other end
    ...
    [Arguments]    ${dst_ip}    ${iface}    ${src_mac}    ${dst_mac}    ${packet_count}    ${packet_type}    ${dst_port}    ${vlan}    ${ip}    ${user}    ${pass}=${None}    ${container_type}=${None}    ${container_name}=${None}
    FOR    ${j}    IN RANGE    8
        ...
        ...    ${ip}    ${user}    ${pass}    ${container_type}    ${container_name}
        Sleep    180s
        ${output}=    Login And Run Command On Remote System    mausezahn ${iface} -B ${dst_ip} -a ${src_mac} -b ${dst_mac} -c ${packet_count} -t ${packet_type} "${dst_port}" -p 1472 -Q ${j}:${vlan} &
        ...    ${ip}    ${user}    ${pass}    ${container_type}    ${container_name}
        Sleep    45s
        ${output}=    Login And Run Command On Remote System    rm -rf /tmp/capture.pcap;tcpdump -l -U -c 30 -w /tmp/capture.pcap -i ${dhcp_server_iface} -e vlan &sleep 10
        ...    ${dhcp_ip}    ${dhcp_user}    ${dhcp_password}    ${None}    ${None}
        Sleep    60s
...
Create downstream traffic with each pbit and capture at other end
    ...
    FOR    ${j}    IN RANGE    8
        ...
        ...    ${dhcp_ip}    ${dhcp_user}    ${dhcp_password}    ${None}    ${None}
        Sleep    180s
        ${output}=    Login And Run Command On Remote System    mausezahn ${dhcp_server_iface} -B ${dst_ip} -a ${dst_mac} -b ${src_mac} -c ${packet_count} -t ${packet_type} "${dst_port}" -p 1472 -Q ${j}:${vlan} &
        ...    ${dhcp_ip}    ${dhcp_user}    ${dhcp_password}    ${None}    ${None}
        Sleep    45s
        ${output}=    Login And Run Command On Remote System    rm -rf /tmp/capture.pcap;tcpdump -l -U -c 30 -w /tmp/capture.pcap -i ${iface} -e vlan &sleep 10
        ...    ${ip}    ${user}    ${pass}    ${container_type}    ${container_name}
        Sleep    60s
        ${output}=    Login And Run Command On Remote System    tcpdump -r /tmp/capture.pcap
        ...    ${ip}    ${user}    ${pass}    ${container_type}    ${container_name}
