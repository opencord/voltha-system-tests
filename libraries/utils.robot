...
Set K8SDateTime
    [Documentation]    Sets data and time for LogCollection keyword.  "k8sdatetime" is required to collect logs for each testcase
    Return From Keyword If    not ${ssh_to_k8s}
    ${k8sdatetime}=    Get Current Datetime On Kubernetes Node    ${k8s_node_ip}    ${k8s_node_user}    ${k8s_node_pass}
    Set Suite Variable    ${k8sdatetime}
...
Log Collection
    ...
    Run Keyword and Ignore Error    Collect Logs
    Return From Keyword If    not ${ssh_to_k8s}
    Log Kubernetes Containers Logs Since Time    ${k8sdatetime}    ${container_list}
