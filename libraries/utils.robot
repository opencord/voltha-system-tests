...
Sanity Test TT MCAST one ONU
    ...
    ${output}=    Login And Run Command On Remote System
    ...    sudo ifconfig ${src['dp_iface_name']} ${src['mcast_rg']} up
    ...    ${src['ip']}    ${src['user']}    ${src['pass']}    ${src['container_type']}    ${src['container_name']}
    ${output}=    Login And Run Command On Remote System
    ...
    ...    sudo iperf -c ${dst['dp_iface_ip_qinq']} -u -T 32 -t 30 -i 1 &
    ...    ${dst['bng_ip']}    ${dst['bng_user']}    ${dst['bng_pass']}    ${dst['container_type']}    ${dst['container_name']}
    
    # Setup iperf on the RG
    ${rg_output}=    Login And Run Command On Remote System
    ...    sudo iperf -s -u -B ${dst['dp_iface_ip_qinq']} -i 1
    ...    ${src['ip']}    ${src['user']}    ${src['pass']}    ${src['container_type']}    ${src['container_name']}
    Log    ${rg_output}
    Should Contain    ${rg_output}    KBytes
