...
Create upstream traffic with each pbit and capture at other end
    ...
    [Arguments]    ${dst_ip}    ${iface}    ${src_mac}
    ...    ${dst_mac}    ${packet_count}    ${packet_type}
    ...
    FOR    ${j}    IN RANGE    8
        ...
        ...    ${ip}    ${user}    ${pass}    ${container_type}    ${container_name}
        Sleep    180s
        ${cmd_mz}=    Set Variable    mausezahn ${iface} -B ${dst_ip} -a ${src_mac} -b ${dst_mac} -c ${packet_count} -t ${packet_type} "${dst_port}" -p 1472 -Q ${j}:${vlan} &
        ${output}=    Login And Run Command On Remote System
        ...    ${cmd_mz}
        ...    ${ip}    ${user}    ${pass}    ${container_type}    ${container_name}
        Sleep    45s
        ${cmd_tcpdump}=    Set Variable    rm -rf /tmp/capture.pcap;tcpdump -l -U -c 30 -w /tmp/capture.pcap -i ${dhcp_server_iface} -e vlan &sleep 10
        ${output}=    Login And Run Command On Remote System
        ...    ${cmd_tcpdump}
        ...    ${dhcp_ip}    ${dhcp_user}    ${dhcp_password}    ${None}    ${None}
        Sleep    60s
...
Create downstream traffic with each pbit and capture at other end
    ...
    FOR    ${j}    IN RANGE    8
        ...
        ...    ${dhcp_ip}    ${dhcp_user}    ${dhcp_password}    ${None}    ${None}
        Sleep    180s
        ${cmd_mz}=    Set Variable    mausezahn ${dhcp_server_iface} -B ${dst_ip} -a ${dst_mac} -b ${src_mac} -c ${packet_count} -t ${packet_type} "${dst_port}" -p 1472 -Q ${j}:${vlan} &
        ${output}=    Login And Run Command On Remote System
        ...    ${cmd_mz}
        ...    ${dhcp_ip}    ${dhcp_user}    ${dhcp_password}    ${None}    ${None}
        Sleep    45s
