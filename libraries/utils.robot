...
Restart openolt when error
    ...
    FOR    ${I}    IN RANGE    0    ${num_olts}
        ...
        ${result}=    Login And Run Command On Remote System
        ...    cat /var/log/openolt.log | grep 'SubscribeIndication failed - StatusCode::INTERNAL : Olt connection complete state indication subscribe failed' | wc -l
        ...    ${olt_ssh_ip}    ${olt_user}    ${olt_pass}    prompt=#

        
        Run Keyword If    ${result}==1    Restart openolt    ${olt_ssh_ip}    ${olt_user}    ${olt_pass}
