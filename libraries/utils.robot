...
Validate ONUs for PON OLT Enable
    ...
    FOR    ${I}    IN RANGE    0    ${num_onus}
        ...
        ...    ${src['user']}    ${src['pass']}    ${src['container_type']}    ${src['container_name']}
        ...    AND    Run Keyword If    ${has_dataplane} and '${dst['ip']}' != '${None}'
        ...    Execute Remote Command    pkill dhcpd
        ...
        ...    ${src['user']}    ${src['pass']}    ${src['container_type']}    ${src['container_name']}
        ...    AND    Run Keyword If    ${has_dataplane} and '${dst['ip']}' != '${None}'    Delete Interface on Remote Host
        ...    ${dst['dp_iface_name']}.${src['s_tag']}    ${dst['ip']}    ${dst['user']}    ${dst['pass']}
        ...    ${dst['container_type']}    ${dst['container_name']}
        ...    AND    Run Keyword If    ${has_dataplane} and "${bng_ip}" != "${NONE}" and "${bng_user}" != "${NONE}" and "${bng_pass}" != "${NONE}"
