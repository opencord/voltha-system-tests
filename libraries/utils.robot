...
Create upstream traffic with each pbit and capture at other end
    ...
    FOR    ${j}    IN RANGE    8
        ${output}    ${stderr}    ${rc}=    Execute Remote Command    pkill -2 mausezahn
        ...    ${ip}    ${user}    ${pass}    ${container_type}    ${container_name}
        Sleep    10s
        ...
        #Sleep    45s
        ${output}    ${stderr}    ${rc}=    Execute Remote Command
        ...    timeout 30 tcpdump -l -U -c 30 -w /tmp/capture.pcap -i ${dst_iface} -e vlan
        ...    ${dhcp_ip}    ${dhcp_user}    ${dhcp_password}
        ${output}    ${stderr}    ${rc}=    Execute Remote Command    pkill -2 mausezahn
        ...    ${ip}    ${user}    ${pass}    ${container_type}    ${container_name}
        Should Match Regexp    ${output}    , p ${j},.*\\.9999: UDP,
    END
