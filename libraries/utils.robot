...
Start Log Capture
    [Arguments]    ${label}
    [Documentation]    Start logging all containers to a temporary file
    ${kail_process}=    Start Process    kail    -n    default    -n    voltha    cwd=${container_log_path}
    ...    stdout=${label}-combined.log
    Set Test Variable    ${kail_process}

End Log Capture
    [Documentation]    End logging all containers to a temporary file
    Run    sync
