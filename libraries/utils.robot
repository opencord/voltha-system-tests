...
WPA Reassociate
    ...
    FOR    ${i}    IN RANGE    70
        ${output}=    Login And Run Command On Remote System    wpa_cli -i ${iface} status | grep SUCCESS    ${ip}    ${user}    ${pass}
        ...    ${container_type}    ${container_name}
...
Validate Authentication After Reassociate
    ...
    ...    ${container_name}=${None}
    [Documentation]    Executes a particular reassociate request on the RG using wpa_cli.
    ...    auth_pass determines if authentication should pass
    ${wpa_log}=    Catenate    SEPARATOR=.    /tmp/wpa    ${iface}    log
    ${output}=    Login And Run Command On Remote System    truncate -s 0 ${wpa_log}; cat ${wpa_log}    ${ip}    ${user}    ${pass}    ${container_type}
    ...    ${container_name}
    Log    ${output}
    ...
    ...    ${wpa_log}    ${iface}.*authentication completed successfully    ${ip}    ${user}    ${pass}    ${container_type}
    ...    ${container_name}
    Run Keyword If    '${auth_pass}' == 'False'    Sleep    20s
    Run Keyword If    '${auth_pass}' == 'False'    Check Remote File Contents    False    /tmp/wpa.log    ${iface}.*authentication completed successfully    ${ip}
...
Send Dhclient Request To Release Assigned IP
    ...
    ${result}=    Login And Run Command On Remote System    dhclient -nw -r ${iface} && rm ${path_dhcpleasefile}/dhclient.*    ${ip}    ${user}    ${pass}    ${container_type}
    ...    ${container_name}
    Log    ${result}
...
Check Remote File Contents For WPA Logs
    ...
    ${result}=    Login And Run Command On Remote System    cat ${file} | grep '${pattern}' | wc -l    ${ip}    ${user}    ${pass}    ${container_type}
...
Perform Sanity Test
    ...
    FOR    ${I}    IN RANGE    0    ${num_onus}
        ...
        ${onu_port}=    Run Keyword And Continue On Failure    Wait Until Keyword Succeeds    ${timeout}    2s    Get ONU Port in ONOS
        ...    ${src['onu']}    ${of_id}
        ...
        Run Keyword And Continue On Failure    Wait Until Keyword Succeeds    ${timeout}    2s    Verify Eapol Flows Added For ONU    ${ONOS_SSH_IP}
        ...    ${ONOS_SSH_PORT}    ${onu_port}
        ...
        Run Keyword And Continue On Failure    Wait Until Keyword Succeeds    ${timeout}    2    Verify ONU in AAA-Users    ${ONOS_SSH_IP}
        ...    ${ONOS_SSH_PORT}    ${onu_port}
        Run Keyword And Continue On Failure    Wait Until Keyword Succeeds    ${timeout}    2    Execute ONOS CLI Command    ${ONOS_SSH_IP}
...
Perform Sanity Test DT
    ...
    Log    ${List_ONU_Serial}
    Run Keyword    Wait Until Keyword Succeeds    ${timeout}    5s    Validate ONU Flows    ${List_ONU_Serial}    ${onu_flows}

Setup
    [Documentation]    Pre-test Setup
    #test for empty device list
    Test Empty Device List
    Run Keyword If    ${has_dataplane}    Wait Until Keyword Succeeds    120s    10s    Openolt is Up    ${olt_ip}
    ...    ${olt_user}    ${olt_pass}
    Sleep    60s
    ...
    Wait Until Keyword Succeeds    180s    5s    Validate OLT Device    ENABLED    ACTIVE    REACHABLE
    ...    ${olt_serial_number}
...
Validate ONUs After OLT Disable
    ...
    FOR    ${I}    IN RANGE    0    ${num_onus}
        ...
        ...    ${of_id}
        Run Keyword And Continue On Failure    Wait Until Keyword Succeeds    ${timeout}    5s    Validate Device    ENABLED
        ...    DISCOVERED    UNREACHABLE    ${src['onu']}    onu=True    onu_reason=stopping-openomci
        Run Keyword And Continue On Failure    Wait Until Keyword Succeeds    ${timeout}    2s    Verify ONU Port Is Disabled    ${ONOS_SSH_IP}
        ...    ${ONOS_SSH_PORT}    ${onu_port}
...
Repeat Sanity Test
    ...
    FOR    ${I}    IN RANGE    0    ${num_onus}
        ...
        ...    ${src['user']}    ${src['pass']}    ${src['container_type']}    ${src['container_name']}    ${dst['dp_iface_name']}
        ...    ${dst['ip']}    ${dst['user']}    ${dst['pass']}    ${dst['container_type']}    ${dst['container_name']}
        Wait Until Keyword Succeeds    ${timeout}    2s    Run Keyword And Continue On Failure    Validate Subscriber DHCP Allocation    ${ONOS_SSH_IP}
        ...    ${ONOS_SSH_PORT}    ${onu_port}
        Run Keyword and Ignore Error    Get Device Output from Voltha    ${onu_device_id}
        Run Keyword and Ignore Error    Collect Logs
    END
...
Verify ping is succesful except for given device
    ...
    FOR    ${I}    IN RANGE    0    ${num_onus}
        ...
        Run Keyword And Continue On Failure    Wait Until Keyword Succeeds    60s    2s    Check Ping    ${pingStatus}
        ...    ${dst['dp_iface_ip_qinq']}    ${src['dp_iface_name']}    ${src['ip']}    ${src['user']}    ${src['pass']}
...
Announce Message
    ...
    Run Process    kubectl    run    announcer    -ti    --rm    --restart
    ...    Never    --image    ubuntu    bash    --    -c
    ...    echo; sleep 1; echo ${message}; sleep 1; date --rfc-3339\=n ; sleep 1; echo; sleep 1
...
Stop Logging
    ...
    ${test_logfile}=    Run Keyword If    "${container_log_dir}" != "${None}"    Join Path    ${container_log_dir}    ${label}-combined.log
...
Clean Up Linux
    ...
    FOR    ${I}    IN RANGE    0    ${num_onus}
        ...
        ...    ${src['container_type']}    ${src['container_name']}
        Run Keyword And Ignore Error    Kill Linux Process    [d]hclient    ${src['ip']}    ${src['user']}    ${src['pass']}
        ...    ${src['container_type']}    ${src['container_name']}
        Run Keyword If    '${dst['ip']}' != '${None}'    Run Keyword And Ignore Error    Kill Linux Process    [d]hcpd    ${dst['ip']}
        ...    ${dst['user']}    ${dst['pass']}    ${dst['container_type']}    ${dst['container_name']}
        Delete IP Addresses from Interface on Remote Host    ${src['dp_iface_name']}    ${src['ip']}    ${src['user']}    ${src['pass']}    ${src['container_type']}
        ...    ${src['container_name']}
        Run Keyword If    '${dst['ip']}' != '${None}'    Delete Interface on Remote Host    ${dst['dp_iface_name']}.${src['s_tag']}    ${dst['ip']}    ${dst['user']}
...
Should Be Larger Than Or Equal To
    [Arguments]    ${value_1}    ${value_2}
    [Documentation]    Verify that value_1 is >= value_2
    Run Keyword If    ${value_1} < ${value_2}    Fail    The value ${value_1} is not larger than or equal to ${value_2}
...
Should Be Newer Than Or Equal To
    ...
    Run Keyword If    ${unix_v1} < ${unix_v2}    Fail    The value ${value_1} is not newer than or equal to ${value_2}
...
Get Bandwidth Profile Name For Given Subscriber
    [Arguments]    ${subcriber_id}
    [Documentation]    Keyword to get the bandwidth details of the given subscriber
    ${banwidth_profile_output}=    Execute ONOS CLI Command    ${ONOS_SSH_IP}    ${ONOS_SSH_PORT}    volt-programmed-subscribers | grep ${subcriber_id}
    @{banwidth_profile_array}=    Split String    ${banwidth_profile_output}    ,
    Log    ${banwidth_profile_array}
    FOR    ${value}    IN    @{banwidth_profile_array}
        @{row_value}=    Split String    ${value}    =
        ${bandwidth_profile_name}=    Set Variable If    '${row_value[0]}' == ' upstreamBandwidthProfile'    ${row_value[1]}
