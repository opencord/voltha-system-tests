...
Get Available Groupmetrics per Device
    ...
    FOR    ${Item}    IN    @{group_list}
        ...
        ${item_dict}=    Set Validation Operation    ${item_dict}    ${onu_device_id}    ${Item}
...
Set Validation Operation
    ...
    [Arguments]    ${item_dict}    ${onu_device_id}    ${group}
...
Set Validation Operation For Counter
    ...
    ${woPrev}=    Create Dictionary    operator=>=    operand=0
    # for real POD it must be >= previous counter value, in case of BBSim usage we got random values, so check >= 0
    ${wPrev}=     Run Keyword If    ${has_dataplane}    Create Dictionary    operator=>=    operand=previous
    ...    ELSE    Create Dictionary    operator=>=    operand=0
    ${ValidationOperation}=    Create Dictionary    woPrev=${woPrev}    wPrev=${wPrev}
...
Set Validation Operation For Context
    ...
    [Arguments]    ${item_dict}    ${metric}    ${group}
    # todo: fill with real check values!
    ${woPrev}=    Create Dictionary    operator=>=    operand=0
    ${wPrev}=     Create Dictionary    operator===    operand=previous
    ${ValidationOperation}=    Create Dictionary    woPrev=${woPrev}    wPrev=${wPrev}
    Set To Dictionary    ${item_dict['${metric}']}  ValidationOperation=${ValidationOperation}
    [return]    ${item_dict}
...
Set Validation Operation For Gauge
    ...
    # todo: fill with real check values!
    ${woPrev}=    Create Dictionary    operator=>=    operand=0
    ${wPrev}=     Create Dictionary    operator=>=    operand=0
    ${ValidationOperation}=    Create Dictionary    woPrev=${woPrev}    wPrev=${wPrev}
    Set To Dictionary    ${item_dict['${metric}']}  ValidationOperation=${ValidationOperation}
    [return]    ${item_dict}
...
Validate Slice Data
    ...
    FOR    ${Index}    IN RANGE    0    ${SliceLength}
        ...
        ...            True    False
        Validate Metrics Data   ${device_id}    ${title}    ${metrics}    ${hasprev}    ${metric_records}    ${Index}
...
Get Validation Operation
    ...
    ${w_wo_prev}=    Set Variable If    ${has_previous}    wPrev    woPrev
...
Get Previous Value
    ...
    ...                It is assumed slice array will be filled in same order always!
    [Arguments]    ${device_id}    ${title}    ${item}    ${metric_records}    ${SliceIndex}
    ...
    ${metrics}=       Get From Dictionary    ${slice[${SliceIndex}]}    metrics
    ${prev_value}=    Get From Dictionary    ${metrics}    ${item}
    [return]    ${prev_value}

Validate Metrics Data
    [Documentation]    Validates passed metrics
    [Arguments]    ${device_id}    ${title}    ${metrics}    ${has_previous}    ${metric_records}    ${SliceIndex}
    FOR    ${item}    IN    @{metrics.keys()}
        ...
        ...    ${device_id}    ${title}    ${item}    ${metric_records}    ${SliceIndex}
        ...    ELSE    Set Variable    ${validation_value}
        ${current_value}=    Get From Dictionary    ${metrics}    ${item}
        ${result}=    utility.compare    ${current_value}    ${operation}    ${validation_value}
