...
Get Previous Slice Index
    ...
    # additional handling for metric group UNI_Status, me_class_id has to be matched too
    ${me_class_id}=    Run Keyword If    '${title}'=='UNI_Status'    Get From Dictionary    ${metrics}    me_class_id
    ...    ELSE    Set Variable    0
    ...
    FOR    ${Index}    IN RANGE    0    ${prevSliceLength}
        ...
        # additional handling for metric group UNI_Status, me_class_id has to be matched too
        ${prev_me_class_id}=    Run Keyword If    '${title}'=='UNI_Status'   Get From Dictionary    ${prevmetrics}    me_class_id
        ...    ELSE    Set Variable    0
        ${matched}=    Set Variable If   (${entity_id}==${prev_entity_id})and(${me_class_id}==${prev_me_class_id})   True   False
