...
Verify Subscriber Access Flows Added for ONU
    ...
    ${upstream_flow_0_cmd}=    Run Keyword If    ${workflow} == 'DT'
    ...    Catenate    SEPARATOR=
    ...
    ${vlan_vid}=    Set Variable If    ${workflow} == 'DT'    Any    ${c_tag}
    ${flow_vlan_push_cmd}=    Catenate    SEPARATOR=
    ...    flows -s ADDED ${olt_of_id} | grep IN_PORT:${onu_port} | grep VLAN_VID:${vlan_vid} |
    ...
    ${downstream_flow_1_cmd}=    Run Keyword If    ${workflow} == 'DT'
    ...    Catenate    SEPARATOR=
    ...    flows -s ADDED ${olt_of_id} | grep IN_PORT:${nni_port} | grep VLAN_VID:Any |
    ...     grep OUTPUT:${onu_port}
    ...    ELSE
    ...    Catenate    SEPARATOR=
    ...
    ${upstream_flow_ipv4_cmd}=    Run Keyword If    ${workflow} == 'ATT'
    ...    Catenate    SEPARATOR=
    ...    flows -s ADDED ${olt_of_id} | grep IN_PORT:${onu_port} | grep ETH_TYPE=ipv4 |
    ...     grep VLAN_VID:${c_tag} | grep OUTPUT:CONTROLLER
    ${upstream_flow_ipv4_added}=    Run Keyword If    ${workflow} == 'ATT'
    ...    Execute ONOS CLI Command    ${ip}    ${port}    ${upstream_flow_ipv4_cmd}
    # Verify ipv4 dhcp downstream flow
    # Note: This flow will be one per nni per olt
    ${downstream_flow_ipv4_cmd}=    Run Keyword If    ${workflow} == 'ATT'
    ...    Catenate    SEPARATOR=
    ...    flows -s ADDED ${olt_of_id} | grep IN_PORT:${nni_port} | grep ETH_TYPE=ipv4 |
    ...     grep OUTPUT:CONTROLLER
    ${downstream_flow_ipv4_added}=    Run Keyword If    ${workflow} == 'ATT'
    ...    Execute ONOS CLI Command    ${ip}    ${port}    ${downstream_flow_ipv4_cmd}
