...
Verify Meters in ONOS
    ...
    ...    meters ${olt_of_id} | grep rate=${us_cir} | grep burst-size=${us_cbs}
    ...     | grep rate=${us_eir} | grep burst-size=${us_ebs} | grep rate=${us_air}
    ${upstream_meter_added}=    Execute ONOS CLI Command    ${ip}    ${port}
    ...    ${us_meter_cmd}
    Should Not Be Empty    ${upstream_meter_added}
    Sleep    1s
    ...
    ...    meters ${olt_of_id} | grep rate=${ds_cir} | grep burst-size=${ds_cbs}
    ...     | grep rate=${ds_eir} | grep burst-size=${ds_ebs} | grep rate=${ds_air}
    ${downstream_meter_added}=    Execute ONOS CLI Command    ${ip}    ${port}
    ...    ${ds_meter_cmd}
    Should Not Be Empty    ${downstream_meter_added}
