...
Open ONOS SSH Connection
    ...
    SSHLibrary.Set Client Configuration    prompt=REGEXP:[(.)+karaf@root\s>]    loglevel=TRACE
...
Execute ONOS CLI Command use single connection
    ...
    ${connection_entry}=    Get From List   ${connection_list}    ${connection_list_id}
    SSHLibrary.Switch Connection   ${connection_entry.conn_id}
    SSHLibrary.Set Client Configuration    prompt=REGEXP:[(.)+karaf@root\s>]
    ${PassOrFail} Write    ${cmd}
    Log    pass_write: ${PassOrFail}
    ${PassOrFail}    @{result_values}    Run Keyword And Ignore Error    Read Until Prompt    strip_prompt=True
